...
function OneVarTable() {
  ...
  const handleSubmit = (e) => {
    ...
    if(emp.id && emp.fname){
      if(editIndex !== null){
        ...
        setEditIndex(null);
        const i = updateContacts.length+1;
        setEmp({id:i,fname:'',lname:'',number:''});
      }
      else{
      setEmpList([...empList,emp]);
      const updateContacts = [...empList]
      const i = updateContacts.length+2;
      setEmp({id:i,fname:'',lname:'',number:''});
      }
  ...
  const handleDelete = (index) =>{
    ...
    for (var i=index; i<contactsDelete.length;i++){
      contactsDelete[i].id -= 1;
    }
    var i = contactsDelete.length+1;
    setEmp({id:i});
